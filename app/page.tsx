...
export default function Home() {
  ...
  const fullMotivationText = "Tenang bro, tools ini udah gw rancang and riset mendalam tentang apa yg ga lo pernah tau sebelumnya. Sistem bakal otomatis nyaring ranjau spambot kurator agensi microstock global, mastiin amunisi prompt lo steril, anti-reject, dan siap nge-bom pasar asset digital dunia!";
  ...
  const handleVerifyLicense = async () => {
    ...
    try {
      ...
      // DETEKTIF KUNCI: Sekarang mengunci dan memeriksa kolom 'device_id_prompt' bray!
  ...
  // --- RENDERING INTERFACE 1: POPUP GERBANG DIMENSI ---
  ...
  // --- STATE INTERFACE 2: ROKET MELUNCUR ---
    ...
    return (
      <main className="relative min-h-screen flex flex-col items-center justify-center bg-[#070A13] text-slate-100 p-4 overflow-hidden z-50">
        ...
        <motion.div
          ...
          transition={{ duration: 3.8, times: [0, 0.05, 0.1, 0.18, 0.35, 0.55, 0.85, 1], ease: "easeInOut" }}
  ...
  // --- STATE INTERFACE 3: UTAMA APLIKASI ASLI ---
  return (
    <main className="relative min-h-screen p-4 md:p-6 flex flex-col items-center justify-start z-10 overflow-x-hidden bg-[#0B0F19] text-slate-100 selection:bg-indigo-500/30">
      ...
      <div className="w-full max-w-6xl mb-4 flex flex-row justify-between items-center gap-4 z-10 border-b border-slate-800/20 pb-2">
        ...
        <motion.img whileHover={{ scale: 1.08, rotate: 3 }} whileTap={{ scale: 0.95 }} src="/logo-brand.svg" alt="Agunk Desain Logo" className="w-24 h-24 object-contain cursor-pointer select-none" />
      </div>
      ...
      <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-12 gap-6 z-10 items-start">
          ...
          <div className="dark-glass p-6 rounded-2xl flex flex-col gap-4 relative backdrop-blur-xl">
            ...
            <div className="flex flex-col gap-1.5 p-3 bg-indigo-500/5 rounded-xl border border-indigo-500/10 w-full">
              <div className="flex justify-between items-center w-full">
                ...
                <button type="button" onClick={() => setShowApiKey(!showApiKey)} className="text-slate-400 hover:text-indigo-400 transition-colors p-1 rounded-md hover:bg-slate-800/60 flex items-center gap-1 text-[10px] font-semibold">
                  {showApiKey ? <><EyeOff className="w-3.5 h-3.5" /> Sembunyikan</> : <><Eye className="w-3.5 h-3.5" /> Intip Key</>}
                </button>
              </div>
              {showApiKey ? (
                <textarea value={userApiKey} onChange={(e) => handleApiKeyChange(e.target.value)} rows={5} placeholder="Tempel semua API Key lo di sini bray..." className="w-full bg-slate-950/70 rounded-lg p-2 text-xs text-slate-200 border border-white/5 focus:border-indigo-500 focus:outline-none font-mono resize-y min-h-[120px]" autoFocus />
              ) : (
                <div onClick={() => setShowApiKey(true)} className="w-full bg-slate-950/70 rounded-lg p-2 text-xs font-mono min-h-[120px] whitespace-pre-wrap cursor-pointer border border-white/5 opacity-80 hover:border-indigo-500/30 transition-all select-none">
                  {userApiKey && userApiKey.trim() ? userApiKey.split('\n').map(line => line.trim() ? '••••••••••••••••••••••••••••••••••••••••' : '').join('\n') : <span className="text-slate-600 block leading-relaxed">Tempel semua API Key lo di sini bray...<br /><span className="text-[10px] text-slate-500">(Klik "Intip Key" untuk mulai mengisi)</span></span>}
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              {!imageRef ? (
                <label className="w-full h-16 border-2 border-dashed border-white/10 hover:border-indigo-500/40 rounded-xl flex items-center justify-center gap-2 text-xs text-slate-400 cursor-pointer bg-slate-950/20 transition-all">
                  <ImageIcon className="w-4 h-4 text-indigo-400" /> <span>Klik buat Upload / Tarik Gambar Dimari</span>
                ...
                <div className="relative w-full h-20 bg-slate-950/60 rounded-xl border border-indigo-500/20 p-2 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0"><p className="text-[11px] text-emerald-400 font-bold">Gambar Terdeteksi!</p><p className="text-[9px] text-slate-500 truncate">Mode analisa visual diaktifkan otomatis.</p></div>
                  <button onClick={handleClearImage} className="p-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded-lg transition-all cursor-pointer"><Trash2 className="w-4 h-4" /></button>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="grid grid-cols-3 gap-2">
                {(['Vector', 'PNG', 'Video'] as const).map((cat) => (
                  <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }} key={cat} type="button" onClick={() => setCategory(cat)} className={`py-2 rounded-xl border text-center text-xs font-bold transition-all cursor-pointer ${category === cat ? 'bg-indigo-600/30 border-indigo-500 text-indigo-200 shadow-lg shadow-indigo-500/10' : 'bg-slate-950/20 border-white/5 text-slate-400 hover:bg-slate-950/40'}`}>
                    {cat === 'Vector' && '🎨 Vector'}{cat === 'PNG' && '📦 PNG Asset'}{cat === 'Video' && '🎬 Video'}
                  </motion.button>
                ))}
              </div>
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="grid grid-cols-3 gap-1.5 text-[10px]">
                {['Single Icon', 'Double Icon', '4 Icons Grid', '8 Icons Grid', 'Pattern Graphic', 'Seamless Grid'].map((lay) => (
                  <motion.button whileHover={{ scale: 1.04, filter: "brightness(1.15)" }} whileTap={{ scale: 0.96 }} key={lay} type="button" onClick={() => setLayoutType(lay)} className={`py-1.5 rounded-lg border text-center font-bold transition-all cursor-pointer ${layoutType === lay ? 'bg-purple-600/30 border-purple-500 text-purple-200 shadow-md shadow-purple-500/5' : 'bg-slate-950/20 border-white/5 text-slate-400 hover:bg-slate-950/40'}`}>{lay}</motion.button>
                ))}
              </div>
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <select value={imageRef ? "Auto-Detect Style via AI Vision" : styleType} disabled={!!imageRef} onChange={(e) => setStyleType(e.target.value)} className={`w-full bg-slate-950/60 rounded-xl p-2.5 text-xs border focus:outline-none ${imageRef ? 'text-indigo-400 border-indigo-500/20 bg-slate-950/80 cursor-not-allowed font-bold' : 'text-slate-300 border-white/5 focus:border-indigo-500'}`}>
                {imageRef ? <option>🤖 Ditentukan otomatis oleh AI berdasarkan gambar</option> : STYLES_DATA[category].map((st) => <option key={st} value={st}>{st}</option>)}
              </select>
            </div>
            ...
            <div className="flex flex-col gap-1.5 p-3 bg-white/5 rounded-xl border border-white/5">
              ...
              {category === 'Vector' && (
                <div className="flex flex-col gap-2 mt-1">
                  <select value={backgroundType} onChange={(e) => setBackgroundType(e.target.value)} className="w-full bg-slate-950/60 rounded-lg p-2 text-xs text-slate-300 border border-white/5 focus:outline-none">
                  ...
                  {backgroundType === 'custom' && <input type="text" value={customColor} onChange={(e) => setCustomColor(e.target.value)} placeholder="Misal: Pastel Soft Blue" className="w-full bg-slate-950/80 rounded-lg p-2 text-xs text-slate-200 border border-indigo-500/40 focus:outline-none" />}
                </div>
              )}
              {category === 'PNG' && <p className="text-[11px] text-amber-400/90 mt-1 leading-relaxed">📌 <strong>Ketetapan PNG:</strong> Latar belakang otomatis dikunci ke <strong>Isolated Pure White</strong>.</p>}
              {category === 'Video' && (
                <select value={backgroundType} onChange={(e) => setBackgroundType(e.target.value)} className="w-full bg-slate-950/60 rounded-lg p-2 text-xs text-slate-300 border border-white/5 focus:outline-none mt-1">
                  <option value="natural">Natural Sesuai Objek Latar</option>
                  <option value="chroma">Isolated Chroma Green Screen Studio</option>
                  <option value="black">Solid Deep Black</option>
                </select>
              )}
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <textarea value={imageRef ? "Biarin gw yang analisa gambarnya..." : rawPrompt} disabled={!!imageRef} onChange={(e) => setRawPrompt(e.target.value)} placeholder="Misal: Kucing astronot imut..." className={`w-full h-20 rounded-xl p-3 text-xs border focus:outline-none resize-none font-medium ${imageRef ? 'bg-slate-950/80 border-indigo-500/20 text-indigo-300 italic font-bold cursor-not-allowed opacity-75' : 'bg-slate-950/50 border-white/5 text-slate-200 focus:border-indigo-500'}`} />
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="flex flex-col gap-1">
                ...
                <select value={trend} onChange={(e) => setTrend(e.target.value)} className="bg-slate-950/50 rounded-lg p-1.5 text-[10px] text-slate-300 border border-white/5 focus:outline-none truncate w-full">
                  {TRENDS_LIST.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}
                </select>
              </div>
              <div className="flex flex-col gap-1">
                ...
                <select value={lighting} onChange={(e) => setLighting(e.target.value)} className="bg-slate-950/50 rounded-lg p-1.5 text-[10px] text-slate-300 border border-white/5 focus:outline-none truncate w-full">
                  {LIGHTING_LIST.map((l) => <option key={l} value={l}>{l}</option>)}
                </select>
              </div>
            </div>

            {errorMsg && <div className="p-3 bg-red-500/10 border border-red-500/20 text-red-400 text-xs rounded-xl font-medium">⚠️ {errorMsg}</div>}

            <motion.button whileHover={{ scale: 1.03, boxShadow: "0px 0px 30px rgba(139, 92, 246, 0.6)" }} whileTap={{ scale: 0.97 }} animate={{ backgroundPosition: ["0% 50%", "100% 50%", "0% 50%"] }} transition={{ backgroundPosition: { duration: 4, repeat: Infinity, ease: "linear" } }} onClick={handleGenerate} disabled={isMounted ? (loading || (!imageRef && !rawPrompt.trim()) || !userApiKey.trim()) : false} className="w-full mt-1 py-3 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 bg-[length:200%_auto] disabled:opacity-30 disabled:pointer-events-none text-white text-xs font-extrabold rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer uppercase tracking-wider shadow-lg shadow-purple-500/20">
              {loading ? <><RotateCcw className="w-3.5 h-3.5 animate-spin" /> Membedah Struktur Visual...</> : <><Sparkles className="w-3.5 h-3.5" /> Ledakkan 50 Variasi Prompt</>}
            </motion.button>
          </div>
        </div>
        ...
        <div className="md:col-span-7">
          <AnimatePresence mode="wait">
            {loading ? (
              <motion.div key="loading-canvas" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -30 }} className="dark-glass h-[620px] rounded-2xl flex flex-col items-center justify-center p-8 text-center border-dashed border-white/10 relative overflow-hidden backdrop-blur-md">
                <RotateCcw className="w-12 h-12 text-indigo-400 animate-spin mb-6" />
                <h3 className="text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 via-purple-400 to-pink-500 uppercase tracking-tighter mb-2 font-mono animate-pulse">Mohon bersabar</h3>
                <p className="text-xs text-slate-300 max-w-sm leading-relaxed font-medium">Prompt lo lagi diracik biar tajam, steril, and anti-reject.</p>
              </motion.div>
            ) : outputPrompts ? (
              <motion.div key="output" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} className="dark-glass p-6 rounded-2xl flex flex-col gap-4 max-h-[85vh] overflow-hidden backdrop-blur-xl">
                <div className="flex justify-between items-center border-b border-white/5 pb-3">
                  <h2 className="text-sm font-bold text-slate-200 flex items-center gap-2 pr-2"><CheckCircle2 className="w-4 h-4 text-emerald-400" /> Hasil Penggandaan {outputPrompts.length} Prompt Selesai</h2>
                  <div className="flex items-center gap-2">
                    <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }} onClick={handleCopyAll} className="px-3 py-1.5 bg-emerald-500/10 border border-emerald-500/30 hover:bg-emerald-500/20 rounded-xl text-[10px] text-emerald-400 font-extrabold tracking-wide flex items-center gap-1.5 transition-all cursor-pointer whitespace-nowrap"><ClipboardCopy className="w-3.5 h-3.5" /> {copiedAll ? 'Disalin!' : 'Salin Semua'}</motion.button>
                    <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }} onClick={handleResetCanvas} className="px-3 py-1.5 bg-red-500/10 border border-red-500/30 hover:bg-red-500/20 rounded-xl text-[10px] text-red-400 font-extrabold tracking-wide flex items-center gap-1.5 transition-all cursor-pointer whitespace-nowrap"><Trash2 className="w-3.5 h-3.5" /> Bersihkan</motion.button>
                  </div>
                </div>
                  ...
                  {outputPrompts.map((promptText, idx) => (
                    <div key={idx} className="group relative p-3.5 bg-slate-950/70 rounded-xl border border-white/5 hover:border-indigo-500/30 transition-all flex flex-col gap-2">
                      <div className="flex justify-between items-center z-10 relative">
                        ...
                        <button onClick={() => handleCopySingle(promptText, idx)} className="text-[10px] text-slate-400 group-hover:text-indigo-300 flex items-center gap-1 transition-all cursor-pointer font-bold"><Copy className="w-3" /> Salin</button>
                      </div>
                      <p className="text-xs text-slate-300 leading-relaxed font-mono select-all relative z-10 pr-2">{promptText}</p>
                    </div>
                  ))}
                </div>
              </motion.div>
            ) : (
              <motion.div initial={{ opacity: 0, y: 40, filter: "blur(10px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)" }} transition={{ duration: 0.8, ease: "easeOut" }} className="dark-glass h-[620px] rounded-2xl flex flex-col items-center justify-center p-8 text-center border-dashed border-white/10 relative overflow-hidden backdrop-blur-md">
                <div className="p-4 bg-gradient-to-r from-indigo-500/10 to-purple-500/10 rounded-full text-indigo-400 mb-4 animate-bounce"><Sparkles className="w-6 h-6" /></div>
                <motion.h3 className="text-4xl md:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 via-purple-400 to-pink-500 uppercase tracking-tighter mb-4 select-none filter drop-shadow-[0_15px_15px_rgba(168,85,247,0.25)] min-h-[50px] font-mono">{typedTitle}</motion.h3>
                <div className="text-xs text-slate-300 max-w-md leading-relaxed font-medium min-h-[80px] select-none font-mono tracking-wide px-4">{typedText}</div>
                <p className="text-[10px] text-slate-500 mt-5 italic select-none">Ketik konsep ide lo di panel kiri atau upload image bocoran trend, lalu klik tombol ledakkan!</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
      ...
      <footer className="w-full max-w-6xl mt-12 pt-6 border-t border-slate-900/60 text-center flex flex-col gap-1 z-10 select-none">
        <p className="text-xs font-bold text-slate-500 font-mono tracking-wider">Copyright © agunkdesain 2026</p>
        <p className="text-[10px] font-medium text-indigo-400/80 italic tracking-wide">Temen lo sukses bareng ..</p>
      </footer>
    </main>
  );
}